...
def get_num_returns(f: Callable) -> int:
    ...
    if isinstance(ret_annotation, str) and ret_annotation.lower().startswith("tuple["):
        ...
        try:
            return len(ret_ast.body.slice.value.elts)  # type: ignore[attr-defined]  # ty:ignore[unresolved-attribute]
        # fix in py3.11
        except AttributeError:  # pragma: no cover
            return len(ret_ast.body.slice.elts)  # type: ignore[attr-defined]  # ty:ignore[unresolved-attribute]
    return 1
